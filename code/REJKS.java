...
/**
 * Modifications by: Bernhard Brenner, SBA Research <bbrenner@sba-research.org>
 */
public class REJKS extends KeyStoreSpi {
	...
	//Methods from Bernhard Brenner
	//--------------------------------------------------------------------------------

	public byte[] getCheckSum(byte[] encryptedPKI) throws UnrecoverableKeyException {

		EncryptedPrivateKeyInfo epki;
		try {
			epki = new EncryptedPrivateKeyInfo(encryptedPKI);
		} catch (Exception x) {
			throw new UnrecoverableKeyException(x.getMessage());
		}
	...
	/**
	 * Returns the salt stored in this JKS file.
	 * @return ...
	 * @throws UnrecoverableKeyException 
	 */
	public byte [] getSalt (byte[] encryptedPKI) throws UnrecoverableKeyException {
		try {
	...
	public byte [] getEncryptedPrivKey (String alias) {
		return (byte[]) privateKeys.get(alias);
	}
	 
	 /**
	 *	Tests a Password for correctness using the checksum of the encrypted data stream.
	 */
	public static boolean passwordCheckWithChecksum (byte [] E, byte[] passwd) throws UnrecoverableKeyException {
		try {
			byte[] keystream = new byte[20];
			System.arraycopy(E, 0, keystream, 0, 20);
			byte[] check = new byte[20];
			System.arraycopy(E, E.length - 20, check, 0, 20);

			byte[] key = new byte[E.length - 40];
			...
			while (count < key.length) {
				...
				for (int i = 0; i < keystream.length && count < key.length; i++) {
					key[count] = (byte) (keystream[i] ^ E[count + 20]);
					count++;
				}
			}
			sha.reset();
	...
	public static byte[] decryptKey(byte[] encryptedPKI, byte[] passwd) throws UnrecoverableKeyException {
		try {
			EncryptedPrivateKeyInfo epki = new EncryptedPrivateKeyInfo(encryptedPKI);
			byte[] encr = epki.getEncryptedData();
			byte[] keystream = new byte[20];
	...
	public static byte[] encryptKey(Key key, byte[] passwd) throws KeyStoreException {
		try {
			MessageDigest sha = MessageDigest.getInstance("SHA1");
	...
	public static byte[] charsToBytes(char[] passwd) {
